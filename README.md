...
本项目当前处于比赛迭代开发阶段。首版聚焦 1 个适应症和 2 家企业；补充版在保持 NSCLC 主线不变的前提下扩展至 **3 家医药企业和 39 条人工核验证据**。

## 项目目标

- 统一整理药物、适应症、临床阶段、试验状态和来源信息。
- 对比当前已核验企业在同一疾病方向上的研发证据布局。
...
| 研发证据中心 | 按关键词、企业、药物、临床试验、研究名称和来源ID检索39条人工核验证据，并组织证据链与企业对比 | 已完成 |
| 企业证据画像 | 按归一企业主体展示来源、试验链、监管链、版本构成、独立资料和待确认关系 | 已完成 |
| 研发事件时间轴 | 按真实结构化日期展示论文分析、公司披露、监管事件、证据版本演进和无日期资料 | 已完成并通过本地浏览器验收 |
...
- 疾病/适应症：非小细胞肺癌（NSCLC）。
- 比较企业：恒瑞医药、百济神州（BeOne Medicines，原BeiGene）、阿斯利康（AstraZeneca）。
- 临床试验：计划整理 10–20 条。
- 医学论文：计划整理 10–20 篇。
- 监管信息：计划整理 5–10 条。
...
当前数据范围：恒瑞医药 15 条、百济神州 16 条、阿斯利康 8 条，共 39 条人工核验来源。阿斯利康补充样本覆盖 FLAURA、ADAURA、LAURA、FLAURA2 四项 III 期试验的 ClinicalTrials.gov 登记与 PubMed 主要论文；登记和论文按试验编号配对，不重复计为多项试验。

阿斯利康补充数据通过 ClinicalTrials.gov v2 API 与 PubMed E-utilities 核验。可重复执行以下命令重建该补充批次：

```bash
python scripts/add_astrazeneca_sources.py
...
旧智能问答、自动化研报、旧白盒溯源、数据库浏览和高级分析的历史代码与 API 继续保留，但不进入比赛导航、启动初始化或默认渲染路径。比赛启动不会请求旧 `/api/bootstrap`、`/api/dashboard`、`/api/profile`、`/api/compare`、`/api/timeline` 或 `/api/chat`，也不展示旧全局企业、年份、排名范围、向量 Top K、DeepSeek 状态和演示动线控件。

Day6 五入口与前半段业务闭环已通过本地浏览器验收：五页切换和高亮、证据中心三页签、页面间跳转及 legacy 请求隔离正常。Grounded QA local 模式对明确 B016 的问题只引用 B015/B016，并区分 B016 的 CHMP 积极意见与 B015 当前 EMA EPAR 正式授权状态。浏览器最终往返切换时 Console 无稳定可复现错误。

## 研发证据中心

Web 端“研发证据中心”用于查询当前 39 条人工核验 NSCLC 资料。页面内部保留三个页签：
...
“循证问答”已成为独立一级入口，但继续复用证据中心原有的 Grounded QA 状态、能力检查、提问、回答、引用、限制、安全拦截和白盒过程，不复制第二套组件。其流程为：
...
上述来源检索、证据链、企业对比和循证问答功能均基于本地 CSV/JSON 配置和 FastAPI 只读服务；只有循证问答 `auto` 模式在本地配置 DeepSeek 后才会尝试调用模型。capabilities 请求失败时，页面仍保留 `local` 本地证据摘要入口。

## 研发决策总览

默认首页“研发决策总览”用于展示当前核验证据样本的真实覆盖情况。该页只调用 `/api/evidence/workbench`，不再依赖旧 `/api/bootstrap`、`/api/dashboard` 或 `/api/profile` 初始化。
...
- 恒瑞医药、百济神州/BeOne Medicines 与阿斯利康/AstraZeneca 的证据覆盖卡片。
...
- 阿斯利康 / AstraZeneca；`阿斯利康` 与 `AstraZeneca` 归一为同一主体。

画像数量来自 `SourceRegistryService`、`EvidenceChainService` 和现有数据版本逻辑动态计算。当前样本中，恒瑞医药为 15 条来源、6 条试验级证据链、0 条药物级监管链；百济神州 / BeOne Medicines 为 16 条来源、4 条试验级证据链、1 条药物级监管链；阿斯利康 / AstraZeneca 为 8 条来源、4 条试验级证据链、0 条药物级监管链。B015/B016 只组成药物级监管链：B015 表示当前 EMA EPAR 正式授权状态，B016 表示 CHMP 积极意见且不是最终批准；B016 不增加 RATIONALE-315 的试验证据数量。

页面同时展示来源类型、研究状态、最新/历史/独立资料、试验链、监管链、独立资料和待确认关系，并提供“查看全部来源”“打开企业对比”“进入循证问答”快捷入口。旧财务画像、雷达评分、风险分、企业排名和 winner 逻辑不进入当前导航或自动请求；双企业证据对比仍保留在“研发证据中心—企业对比”内部页签。
...
主导航“事件时间轴”已重构为“研发事件时间轴”。新页面只请求：

- `GET /api/evidence/timeline`
- `GET /api/evidence/timeline/{company}`

时间轴复用 `SourceRegistryService`、`EvidenceChainService`、`CompanyEvidenceProfileService` 和工作台元信息逻辑，不依赖旧 SQLite、Chroma 或旧 `/api/timeline`。合并后当前 39 条人工核验来源中，动态识别出 16 条具有可用非核验日期：15 条进入核心时间轴，B014 作为 1 条可选辅助证据更新，另有 23 条资料因缺少可用事件日期进入“无日期资料”。默认不显示 B014，也不根据试验编号、标题或当前年份生成试验登记、开始、完成或终止事件。

日期选择规则：PubMed 论文优先使用 `online_publication_date`；其他事件使用 `publication_date`；`data_cutoff_date` 只形成辅助证据更新；`source_last_updated` 只作为页面元信息；`verified_at` 和响应 `generated_at` 均不是研发事件日期。月精度日期保持为 `YYYY-MM`，不会补成虚构的月初日期。

当前核心事件中，恒瑞医药 2 条，百济神州 / BeOne Medicines 9 条，阿斯利康 / AstraZeneca 4 条。B015 的 2023-09-15 事件仍表示 Tevimbra 欧盟初始许可日期；2026-05-27 是当前 EPAR 页面更新时间，当前 EPAR 已将围手术期 NSCLC 适应症列入正式授权范围。B016 显示为 2025-07-24 的“CHMP积极意见，非最终批准”。两者属于 `regulatory:tevimbra-eu-nsclc` 药物级监管链，不作为普通历史/最新替代关系，也不增加 NCT04379635 的唯一试验数。

RATIONALE-304 的 B006/B007 和 RATIONALE-307 的 B008/B009 各形成两条事件、一个唯一试验，并同时提供正向和反向版本关系。B010 只形成一条“中期与最终合并论文”事件。页面支持企业、试验、药物、事件类型、年份筛选和辅助事件开关，并提供查看来源、查看证据链和进入循证问答入口。

Day6 研发事件时间轴本地浏览器人工验收已通过。该次验收是 2026-07-23 合并前 31 条来源快照：默认 11 条核心事件和 19 条无日期资料、两家企业及全部筛选、B015/B016 监管口径、两组版本演进、B014 辅助开关、四类跳转入口、仅请求新 timeline API、Console 和窄屏布局均正常。合并后的 39 条来源及三企业指标已通过服务、API 和前端自动回归；浏览器中的三企业当前数据展示仍应按合并结果复验。该结论不是 Render 线上部署验收。

范围限制：事件数只描述当前核验样本中的可用日期记录，不能解释为企业研发实力、研发活跃度、疗效优劣、成功率或投资价值。本阶段未部署到 Render。
...
Day5 历史部署分支为 `feature/day5-release-deploy`，当时线上验收的是旧导航下的研发证据查询四页签结构。Day6 五入口比赛导航尚未部署 Render，后续需要单独完成浏览器和线上验收。Render Free 实例空闲后可能休眠，首次访问或长时间空闲后的访问可能需要等待冷启动。
...
该接口用于区分比赛核心轻量环境与旧功能能力状态。响应包含 `evidence_workbench_available`、`company_evidence_profile_available` 和 `rd_event_timeline_available`；这些能力只依赖当前 CSV/JSON 证据服务，不因空 SQLite 而关闭。证据工作台可用时返回 `default_page=today`，首页默认进入“研发决策总览”，只请求 `/api/evidence/workbench`。即使 `legacy_features_available=true`，比赛启动也不会自动请求 `/api/bootstrap`、`/api/profile` 或 `/api/dashboard`；旧功能仅保留内部兼容能力，不决定比赛导航和默认渲染。
...
- [x] 完成统一证据查询服务、FastAPI 证据接口和 Web 端研发证据中心。
- [x] 完成工作台、企业画像、事件时间轴、证据中心和循证问答的比赛前半段闭环。
- [ ] 建立可追溯的真实数据集导入流程。
- [ ] 完成业务评测、演示缓存和展示材料。
