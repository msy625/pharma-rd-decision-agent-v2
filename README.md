...
- **公开环境保护**：公开 Render 环境默认关闭大模型生成，保留无需 API Key 的本地证据分析能力。
...
`/health` 只检查进程存活；`/ready` 检查比赛核心数据是否可读，并返回数据版本和来源数。公开环境在 `render.yaml` 中设置 `GROUNDED_QA_LLM_ENABLED=false`，大模型生成默认关闭。若未来确需启用，只能在 Render 控制台以环境变量保存密钥，并同时评估公网配额、并发和滥用风险，不能将真实密钥写入仓库。
...
- [x] 提供本地无密钥模式、可选 DeepSeek 模式和公开环境默认关闭策略。
- [x] 提供 Render 轻量部署配置、健康检查与就绪检查。
- [ ] 持续扩充独立监管来源，并人工解决当前待确认关系。
- [ ] 在统一数据、随机种子和评测条件下继续完善业务评测与展示材料。
