...
| 研发证据中心 | 按关键词、企业、药物、临床试验、研究名称和来源ID检索31条人工核验证据，并组织证据链与企业对比 | 已完成 |
...
Day6 五入口与前半段业务闭环已通过本地浏览器验收：五页切换和高亮、证据中心三页签、页面间跳转及 legacy 请求隔离正常。Grounded QA local 模式对明确 B016 的最终批准问题只引用 B015/B016，并区分 Tevimbra 欧盟初始许可与 CHMP 积极意见。浏览器最终往返切换时 Console 无稳定可复现错误。

## 研发证据中心

Web 端“研发证据中心”用于查询第一版冻结的 31 条人工核验 NSCLC 资料。页面内部保留三个页签：
...
时间轴复用 `SourceRegistryService`、`EvidenceChainService`、`CompanyEvidenceProfileService` 和工作台元信息逻辑，不依赖旧 SQLite、Chroma 或旧 `/api/timeline`。当前 31 条人工核验来源中，动态识别出 12 条具有可用非核验日期：11 条进入核心时间轴，B014 作为 1 条可选辅助证据更新，另有 19 条资料因缺少可用事件日期进入“无日期资料”。默认不显示 B014，也不根据试验编号、标题或当前年份生成试验登记、开始、完成或终止事件。
...
当前核心事件中，恒瑞医药 2 条，百济神州 / BeOne Medicines 9 条。B015 显示为 2023-09-15 的“Tevimbra欧盟初始许可”，2026-05-27 仅是页面更新时间；B016 显示为 2025-07-24 的“CHMP积极意见，非最终批准”。两者属于 `regulatory:tevimbra-eu-nsclc` 药物级监管链，不作为普通历史/最新替代关系，也不增加 NCT04379635 的唯一试验数。
...
Day6 研发事件时间轴本地浏览器人工验收已通过：默认 11 条核心事件和 19 条无日期资料、两家企业及全部筛选、B015/B016 监管口径、两组版本演进、B014 辅助开关、四类跳转入口、仅请求新 timeline API、Console 和窄屏布局均正常。该结论不是 Render 线上部署验收。
