...
class DecisionAgentFrontendStaticTest(unittest.TestCase):
    ...
    def test_07_three_golden_demo_cards_run_immediately(self):
        ...
        self.assertIn("运行此案例", self.evidence_template)
    ...
    def test_10_decision_object_is_structurally_mapped(self):
        ...
        for label in [
            "决策摘要", "核心发现", "比较维度", "证据成熟度与可追溯性",
            "风险提示", "证据缺口", "下一步证据行动",
            "当前样本能支持的结论", "当前样本不能支持的结论",
            "Agent 综合说明",
        ]:
    ...
    def test_11_featured_citations_are_preferred_and_full_citations_toggle(self):
        for snippet in [
            ...
            "查看全部 '+agentFullCitations.length+' 条证据",
        ]:
            self.assertIn(snippet, self.evidence_component)
        for text in ["代表性证据", "全部证据"]:
            self.assertIn(text, self.evidence_template)
        for text in ["查看全部", "收起全部证据", "agent_allCitationsToggleText"]:
    ...
    def test_12_citations_show_trace_and_safe_links(self):
        for snippet in [
            ...
            self.assertIn(snippet, self.evidence_component)
        for text in ['data-produced-by-steps', 'target="_blank"', 'rel="noopener noreferrer"', "查看来源详情"]:
            self.assertIn(text, self.evidence_template)
    ...
    def test_13_safety_refusal_is_separate_from_normal_decision(self):
        ...
        self.assertIn("安全拒答", self.evidence_template)
    ...
    def test_14_data_insufficient_is_separate_from_system_error(self):
        ...
        self.assertIn("缺少的条件或证据", self.evidence_template)
    ...
    def test_15b_capabilities_value_mapping_uses_agent_value_helper(self):
        ...
        self.assertIn(
            "fields.push({label:'data_version', value:this._agentValueText(cap.data_version)})",
            self.evidence_component,
        )
        self.assertIn(
            "fields.push({label:'safety_scope', value:this._agentValueText(cap.safety_scope)})",
            self.evidence_component,
        )
