...
class EvidenceWorkbenchServiceTest(unittest.TestCase):
    ...
    def test_01_summary_counts_are_dynamic_from_verified_services(self):
        ...
        self.assertEqual(summary["source_count"], 39)
        self.assertEqual(summary["verified_source_count"], 39)
        self.assertEqual(summary["company_count"], 3)
        self.assertEqual(summary["trial_chain_count"], 14)
        self.assertEqual(summary["regulatory_chain_count"], 1)
        self.assertEqual(summary["latest_count"], 8)
        self.assertEqual(summary["historical_count"], 2)
        self.assertEqual(summary["independent_count"], 29)
    ...
    def test_02_beone_aliases_are_one_company_subject(self):
        ...
        self.assertIn("阿斯利康/AstraZeneca", names)
    ...
    def test_03_b016_regulatory_context_does_not_increase_trial_count(self):
        ...
        self.assertEqual(self.workbench["summary"]["trial_chain_count"], 14)
        self.assertEqual(len([chain for chain in chains if chain["chain_type"] == "trial"]), 14)
    ...
    def test_04_distribution_and_metadata_are_present(self):
        ...
        self.assertEqual(metadata["data_scope"], "verified_nsclc_multi_company_sample")
        self.assertTrue(metadata["data_version"].startswith("sha256:"))
        self.assertEqual(metadata["latest_verified_at"], "2026-07-23")
